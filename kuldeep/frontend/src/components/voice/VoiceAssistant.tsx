...
export const VoiceAssistant: React.FC<VoiceAssistantProps> = ({
  ...
  const generateVoiceResponse = useCallback((command: VoiceCommand) => {
    ...
    switch (command.action) {
      ...
      case 'describe':
        if (currentRoom) {
          response = getRoomDescription(currentRoom);
        } else {
          response = 'I can see you\'re viewing a property room. Look around to explore the space, or ask me to go to a specific room.';
        }
        break;
  ...
  const getRoomDescription = (room: string): string => {
    const descriptions: Record<string, string> = {
      kitchen: 'This is a modern kitchen space. Look around to see the appliances, countertops, and overall layout. Notice the storage options and workspace available.',
      bedroom: 'This bedroom offers a comfortable sleeping space. Observe the natural lighting, room dimensions, and any built-in storage features.',
      bathroom: 'This is the bathroom area. You can see the fixtures, vanity, and overall design. Check out the lighting and storage options.',
      living_room: 'This is the main living area where you can relax and entertain guests. Notice the space flow, lighting, and any architectural features.'
    };
    
    return descriptions[room] || 'This is a nice room in the property. Take a moment to look around and explore the space.';
  };
